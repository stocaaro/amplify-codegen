...
export function codegen(options: AmplifyGenerateOptions): string {
  ...
  if (schemaInstance && documents.length > 0 && shouldValidateDocumentsAgainstSchema(skipDocumentsValidation)) {
    ...
    const extraFragments: { importFrom: string; node: DefinitionNode }[] = pickFlag('externalFragments', options.config) || [];
...
function validateDuplicateDocuments(files: Types.DocumentFile[]) {
  ...
  function addDefinition(file: Source, node: DefinitionNode & { name?: NameNode }, deduplicatedDefinitions: Set<DefinitionNode>) {
